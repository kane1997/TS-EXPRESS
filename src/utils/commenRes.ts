...
function commonRes(res: Response, data: unknown, options?: ResOption) {
    options = Object.assign({ type: Code[1] }, options || {}) // 默认success
    ...
    if (resStatus === undefined) {
        // 根据状态设置状态码
        resStatus = type === Code[1] ? 200 : 409
    }

    // 响应参数
    const sendRes: { success: number; data: unknown; message?: unknown } = {
        success: Code[type as codeType],
        data,
    }
    // 响应描述
